...
@Entity
@Table(
        name = "finance_settlement_daily_settlement", // 테이블명 명시
        indexes = {
                @Index(name = "idx_daily_seller", columnList = "seller_id"),   // 판매자별 조회 (필수)
                @Index(name = "idx_daily_daily_settlement_status", columnList = "daily_settlement_status"),      // "PENDING" 상태인 것만 배치로 뽑아야 함 (필수)
                @Index(name = "idx_daily_target", columnList = "target_date")  // "2026-01" 정산 내역 조회용
        }
)
@Getter
...
public class DailySettlement extends BaseTimeEntity {
    ...
    @Column(name = "daily_settlement_status", nullable = false, length = 30)
    private DailySettlementStatus dailySettlementStatus;
    ...
    @Builder
    public DailySettlement(Long sellerId, LocalDate targetDate, Long totalAmount, Long feeAmount,
        ...
        this.dailySettlementStatus = DailySettlementStatus.PENDING; // 초기 상태는 무조건 PENDING
    ...
    public void startPayout() {
        this.dailySettlementStatus = DailySettlementStatus.PROCESSING;
    }

    // 배치 끝나고, payment 모듈에 알림 후 포인트 지급 완료
    public void successPayout() {
        this.dailySettlementStatus = DailySettlementStatus.COMPLETED;
    }
    ...
    public void failPayout(String errorMessage) {
        ...
        if(retryCount >= 3){
            this.dailySettlementStatus = DailySettlementStatus.FAILED;
        }
        else{
            this.dailySettlementStatus = DailySettlementStatus.PENDING;
        }
    }
